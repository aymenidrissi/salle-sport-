...
@section('content')
    <div class="mx-auto max-w-6xl">
        <div class="mb-5 flex items-center justify-between gap-3">
            <h1 class="text-2xl font-bold tracking-tight text-zinc-900">Vidéos</h1>
            <a href="{{ route('admin.videos.create') }}" class="rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white hover:bg-brand/90">
                + Ajouter une vidéo
            </a>
        </div>
        ...
        <div class="overflow-x-auto rounded-[10px] border border-zinc-200 bg-white shadow-sm">
            <table class="w-full min-w-[760px] text-left text-sm">
                <thead class="bg-zinc-50 text-xs uppercase tracking-wide text-zinc-500">
                    <tr>
                        <th class="px-4 py-3 font-medium">Vidéo</th>
                ...
                <tbody class="divide-y divide-zinc-100">
                    @forelse ($videos as $video)
                        <tr>
                            ...
                            <td class="px-4 py-3">
                                <a href="{{ $video->url }}" target="_blank" rel="noopener noreferrer" class="max-w-[320px] block truncate text-blue-600 hover:underline">
                                    {{ $video->url }}
                                </a>
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.videos.edit', $video) }}" class="rounded-lg p-2 text-zinc-500 hover:bg-zinc-100 hover:text-zinc-700" title="Modifier">
                                    ...
                                    <form method="POST" action="{{ route('admin.videos.destroy', $video) }}" onsubmit="return confirm('Supprimer cette vidéo ?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-4 py-10 text-center text-zinc-500">Aucune vidéo pour le moment.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
